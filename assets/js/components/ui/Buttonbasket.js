const temp = `
    <div class="grid grid-cols-1 gap-1 grid-cols-1">
        ...
        <a v-if="!mark" v-on:click="insertIntoBasket(biblioId, $event)" href="#addToBasket" class="w-12/12 mr-1 btn btn-outline-danger hover:bg-danger-700 text-black font-super-bold text-xs py-2 px-2 m-1 rounded-lg">
        ...
        <a v-if="mark" v-on:click="removeFromBasket(biblioId, $event)" title="Klik untuk menghapus dari keranjang" href="#removeFromBasket" class="w-12/12 mr-1 btn btn-secondary text-white font-super-bold text-xs py-2 px-2 m-1 rounded-lg">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="inline-block mr-1 bi bi-book" viewBox="0 0 16 16">
